...
typedef struct osprd_info {
	...
	//accessory fields for design problem
...
/* atoi(char *str) 
 * convert a C string into an int
 */
int atoi(char *str);
...
//	eprintk("Should process request...\n");
	...
	// eprintk("**********req->current_nr_sectors: %d.\n", req->current_nr_sectors);
	// eprintk("**********req->sector: %d.\n", req->sector);
	...
	if(rq_data_dir(req) == READ) {
		// eprintk("*******CONDUCTED A READ*********\n");
	//	osp_spin_lock(&(d->mutex));
	//	d->info_flag = 0;
	//	d->info_byte_offset = dataOffset;
	//	d->info_byte_size = dataSize;
	...
	//	wake_up_all(&(d->blockq));
		...
		// eprintk("*******CONDUCTED A WRITE*********\n");
		osp_spin_lock(&(d->mutex));
		// eprintk("check req->buffer 500: %d\n.", req->buffer[500]);
		for(i=0;i<dataSize;i++){
			if(start==-1 && ((d->data+dataOffset)[i] != (req->buffer)[i])) start=i;
			if((d->data+dataOffset)[i] != (req->buffer)[i]) {
				end=i;
			}
		}
		if(d->info_flag == 0 && start != -1) {
			d->info_byte_offset = dataOffset+start;
			d->s_change = 1;
		}
		else if(d->info_flag == 0) d->info_byte_offset = dataOffset;
		d->info_flag = 1;
		// d->info_byte_offset = dataOffset+start;
		if(end!=-1){
			d->info_byte_size = dataOffset + end - d->info_byte_offset+1;
		}
		if(!d->s_change){
			if(start==-1 && end==-1){
				d->info_byte_size=0;
		...
		// eprintk("DATASIZE: %d,,,,%d,,,,%d,,%d,,%d, %d\n",d->info_byte_offset,d->info_byte_size,dataOffset,start,end,d->info_flag);
		wake_up_all(&(d->blockq));
		osp_spin_unlock(&(d->mutex));
