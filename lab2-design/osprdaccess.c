...
int main(int argc, char *argv[])
{
 ...
 flag:
	...
	msg_t u_info;
	u_info.w_type = w_type;
	u_info.u_offset = u_offset;
	u_info.u_end = u_end;

	if(w_type != '\0') {
		// if(!(isNumber(u_offset) && isNumber(u_end))) {
		// 	perror("Message: INPUT INVALID");
		// 	exit(1);
		// }
		ioctl(devfd, OSPRDWATCHER, &u_info);
		perror("QUITING RAMDISK WATCHER!");
		exit(1);
	}
