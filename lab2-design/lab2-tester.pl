...
# interrupting a blocked lock doesn't destroy the wait order
    ...
    [  #dead lock detection between 2 processes
       #p1->p2->p1
      'echo ucla | ./osprdaccess -w 4 /dev/osprda;'.
      '(sleep 0.2; ./osprdaccess -r 4 -l -d 0.5 /dev/osprda /dev/osprda) & '.
      '(sleep 0.2; echo sjtu | ./osprdaccess -w 4 -l 0.2 -d 0.5 /dev/osprda)',
      "ioctl OSPRDIOCACQUIRE: Resource deadlock avoided"
    ],

    #20
    [  #dead lock detection between 4 processes
       # p1->p2->p3->p4->p1
      'echo "" | ./osprdaccess -w 0 /dev/osprda;'.
      '(sleep 0.2; ./osprdaccess -r 0 -l -d 0.7 /dev/osprda /dev/osprda) & '.
      '(sleep 0.2; echo "" | ./osprdaccess -w 0 -l 0.1 -d 0.3 /dev/osprda) & '.
      '(sleep 0.2; ./osprdaccess -r 0 -l 0.2 -d 0.3 /dev/osprda) & '.
      '(sleep 0.2; echo "" | ./osprdaccess -w 0 -l 0.3 -d 0.5 /dev/osprda)',
      "ioctl OSPRDIOCACQUIRE: Resource deadlock avoided"
    ],
    );
