    ...
    while (1)
    {
        ...
        switch (c)
        {
            ...
            case RDONLY:
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--RDONLY: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--RDONLY: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("--RDONLY: Failed to get CPU time info.\n");
                    }
                }
                break;
            case COMMAND: //command
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--COMMAND: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--COMMAND: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("--COMMAND: Failed to get CPU time info.\n");
                    }
                }
                break;
            case WAIT:{
//Wait for all commands to finish. As each finishes, output its exit status, and a copy of the command (with spaces separating arguments) to standard output.
                successgetcpu = 1;
                double totalchildutime=0.0;
                double totalchildstime=0.0;
                if(getrusage(RUSAGE_SELF, &parentcpubefore) != 0) successgetcpu = 0;
                ...
                for(k = 0; k < nextjob; k++) {
                    successgetchildcpu = 1;
                    if(getrusage(RUSAGE_CHILDREN, &childcpubefore) != 0) successgetchildcpu = 0;
                    returnpid =  waitpid(-1, &status, 0);
                    if(getrusage(RUSAGE_CHILDREN, &childcpuafter) != 0) successgetchildcpu = 0;
                    ...
                    for(j=0; j < nextjob; j++) {
                        if(job_list[j].pid == returnpid) {
                            ...
                            printf("\n");
                            if(profile_flag) {
                                printf("Performance stat of the child process above: \n");
                                if(successgetchildcpu == 1) {
                                    printf("User CPU time: %f\n", timeconvert(&childcpubefore.ru_utime, &childcpuafter.ru_utime));
                                    printf("System CPU time: %f\n", timeconvert(&childcpubefore.ru_stime, &childcpuafter.ru_stime));
                                    totalchildstime += timeconvert(&childcpubefore.ru_stime, &childcpuafter.ru_stime);
                                    totalchildutime += timeconvert(&childcpubefore.ru_utime, &childcpuafter.ru_utime);
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--WAIT: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--WAIT: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                        printf("Total child user CPU time: %f\n", totalchildutime);
                        printf("Total child systme CPU time: %f\n", totalchildstime);
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
...
//                printf("pipe is %d, %d \n", fd[1], fd[0]);
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--pipe: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--pipe: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
...
//                int b = *a;
            ...
            case CLOSE: {
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--close: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--close: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
            ...
            case CATCH: {
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--catch: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--catch: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
            ...
            case IGNORE: {
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--ignore: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--ignore: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
            ...
            case DEFAULT: {
                ...
                if(profile_flag) {
                    if(successgetcpu == 1) {
                        printf("--default: User CPU time: %f\n", timeconvert(&parentcpubefore.ru_utime, &parentcpuafter.ru_utime));
                        printf("--default: System CPU time: %f\n", timeconvert(&parentcpubefore.ru_stime, &parentcpuafter.ru_stime));
                    } else {
                        printf("Failed to get CPU time info.\n");
                    }
                }
...
double timeconvert(struct timeval *before, struct timeval *after) {
    double aftertime = (double)after->tv_sec + (double)after->tv_usec/1000000;
    double beforetime = (double)before->tv_sec + (double)before->tv_usec/1000000;
