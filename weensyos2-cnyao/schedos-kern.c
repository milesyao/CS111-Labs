...
start(void)
{
	...
	l = (uint16_t *) 0x198004;
	memset(l, 0, sizeof(uint16_t));
	memset(proc_array, 0, sizeof(proc_array));
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
		proc_array[i].p_state = P_EMPTY;
		proc_array[i].p_priority = 1;
		proc_array[i].p_ntickets = 1;
		proc_array[i].p_counter = 0;
	}

	// Set up process descriptors (the proc_array[])
	//HAVE A HIHG LEVEL IDEA
	for (i = 1; i < NPROCS; i++) {
	...
	proc_array[1].p_counter = 1;//EXERCISE 4B
	run(&proc_array[1]);
...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_S_LOTTERY:
		current->p_ntickets = reg->reg_eax;
		run(current);
	default:
		while (1)
			/* do nothing */;
...
schedule(void)
{
	...
	if (scheduling_algorithm == 0) {
		...
		int chosen, former, j;
		int count = 0;
		chosen = current->p_pid;
		former = current->p_pid;
		if(proc_array[current->p_pid].p_state != P_RUNNABLE) {
			proc_array[current->p_pid].p_priority = 0x7FFFFFFF;
		}
		while(1) {
			for(j = former + 1; j < NPROCS + former; ++j) {
				if(j == NPROCS) continue;
				if(proc_array[j % NPROCS].p_state == P_RUNNABLE) {
					if(proc_array[chosen].p_priority > proc_array[j % NPROCS].p_priority) {
						chosen = j % NPROCS;
						count = 1;
					} else if(proc_array[chosen].p_priority == proc_array[j % NPROCS].p_priority) {
						if(count == 0) {
							chosen = j % NPROCS;
							count = 1;
						}
		...
		int j;
		while(1) {
			for (j = pid; j < NPROCS + pid; ++j) {
				if(j % NPROCS != 0 && proc_array[j % NPROCS].p_state == P_RUNNABLE) {
					if(current->p_counter == proc_array[j % NPROCS].p_priority) {
						current->p_counter = 0;
						continue;
					}
					proc_array[j % NPROCS].p_counter++;
					run(&proc_array[j % NPROCS]);
				}
			}
		}
		...
		int ticket_sum = 0;
		for(j = 1; j < NPROCS; j++) {
			ticket_sum += proc_array[j].p_ntickets;
		}
		r = ((r * 7621) + 1) % 32768;
		rd = r % ticket_sum + 1;
		ticket_sum = 0;
